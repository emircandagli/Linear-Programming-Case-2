...
"""pre-emptive goal programming z1 and z2 order part 2"""

model.vOD = pyo.Var(bounds = (0.0, None))
...
def eFirst_Objective(model):
    return model.vTotal_Cost == 2455450

model.eFirst_Objective = pyo.Constraint(rule = eFirst_Objective)
...
def eDeviation(model):
    return model.vIdle <= 2200 + model.vOD

model.eDeviation = pyo.Constraint(rule = eDeviation)

def oDeviation(model):
    return model.vIdle

model.oDeviation = pyo.Objective(rule = oDeviation, sense = pyo.minimize)

#Export the open form of the model with the user defined labels
model.write('model_labels_pre-emptive_z1.lp', io_options={'symbolic_solver_labels': True})
#Export the open form of the model without the user defined labels
model.write('model_nolabels_pre-emptive_z1.lp', io_options={'symbolic_solver_labels': False})
