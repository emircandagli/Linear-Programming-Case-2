...
"""weighted goal programming for w1 = 0.25, w2 = 0.75"""

model.vOD1 = pyo.Var(bounds = (0.0, None))
model.vOD2 = pyo.Var(bounds = (0.0, None))
...
def eDeviation1(model):
    return model.vIdle <= 3200000 + model.vOD1

model.eDeviation1 = pyo.Constraint(rule = eDeviation1)

def eDeviation2(model):
    return model.vIdle <= 2200 + model.vOD2

model.eDeviation2 = pyo.Constraint(rule = eDeviation2)

def oDeviation(model):
    return ((0.25 * model.vOD1 / 2614618) + (0.75 * model.vOD2 / 3171)) 
